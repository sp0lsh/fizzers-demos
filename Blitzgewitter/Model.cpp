         ...
         case 'o':
            {
               if(!object.empty())
               {
                  so.triangle_count=triangles.size()-so.first_triangle;
               ...
               object = c;
               so.name=object;
               so.first_triangle=triangles.size();
               so.triangle_count=0;
            }
            break;
...
void model::Mesh::getAABB(Vec3& box_min, Vec3& box_max)
{
   box_min = Vec3(1e9, 1e9, 1e9);
   box_max = Vec3(-1e9, -1e9, -1e9);
